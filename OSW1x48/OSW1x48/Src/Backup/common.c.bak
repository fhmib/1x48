...
uint8_t Is_Value_Approximate(int32_t src, int32_t dst, double factor)
{
  ...
  double dst_rate = (double)dst / 0x1000;
...
#endif
osStatus_t get_32_from_cali_area(uint16_t addr, uint32_t *pval)
{
  memcpy(pval, (void*)(CALI_ADDRESS + addr), 4);
  return osOK;
}

osStatus_t write_32_to_cali_area(uint16_t addr, uint32_t data)
{
  if (FLASH_If_Write(CALI_ADDRESS + addr, &data, 1) != FLASHIF_OK) {
    return osError;
  }
  return osOK;
}

osStatus_t Cali_Write(uint16_t mem_addr, uint8_t *buf, int32_t length)
{
  if (FLASH_If_Write(CALI_ADDRESS + mem_addr, (uint32_t*)buf, length / 4) != FLASHIF_OK) {
    return osError;
  }
  return osOK;
}

osStatus_t Cali_Read(uint16_t mem_addr, uint8_t *buf, int32_t length)
{
  uint32_t val, cplt_bytes = 0;

  while (cplt_bytes < length) {
    get_32_from_cali_area(mem_addr + cplt_bytes, &val);
    BE32_To_Buffer(val, buf + cplt_bytes);
    cplt_bytes += 4;
  }
  // memcpy(buf, (void*)(CALI_ADDRESS + mem_addr), length);
  return osOK;
}
...
osStatus_t RTOS_DAC5504_SPI1_Write(uint8_t mode, uint16_t val)
{
  osStatus_t status;
  uint8_t txbuf[2];
...
osStatus_t RTOS_DAC5504_SPI2_Write(uint8_t mode, uint16_t val)
{
  osStatus_t status;
  uint8_t txbuf[2];
...
osStatus_t RTOS_DAC5504_SPI5_Write(uint8_t mode, uint16_t val)
{
  osStatus_t status;
  uint8_t txbuf[2];
...
osStatus_t RTOS_DAC5504_SPI6_Write(uint8_t mode, uint16_t val)
{
  osStatus_t status;
  uint8_t txbuf[2];
